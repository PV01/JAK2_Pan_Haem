...
JAK2_values<-JAK2_values %>%
                       filter((RESULT>0.22 & RESULT<0.28)|(RESULT>0.45 & RESULT<0.55)|(RESULT>0.85 & RESULT<1.1)|(RESULT>1.9 & RESULT<2.1)|(RESULT>2.8 & RESULT<3.2)) %>%
                       arrange(RESULT) %>%
                       select(-SIGN_BY)
...
JAK2_depth<-function(labnr){
  ...
  rl <- IRangesList()
  rl[[chromosome]] <- IRanges(start=5073770, end=5073770, names=chromosome)
  ...
  return(data.frame(LABNO=labnr, JAK2V617_coverage=p$count))
}
...
NGS_JAK2V617_coverage<-data.frame()
for (i in 1:nrow(JAK2_values_NGS)) {
  NGS_JAK2V617_coverage<-rbind(NGS_JAK2V617_coverage, JAK2_depth(JAK2_values_NGS$LABNO[i]))
}

  
JAK2_values_NGS_annot<-JAK2_values_NGS %>%
                                left_join(NGS_coverage, by=c("LABNO"="LABNO")) %>%
                                left_join(NGS_JAK2V617_coverage, by=c("LABNO"="LABNO"))    
